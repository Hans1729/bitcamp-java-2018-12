...
//   트랜잭션 시작 할 때 수동 커밋의 SqlSession 객체의 auto commit을 false로 지정한다.
//   트랜잭션을 종료할 때 SqlSession객체를 close() 한다.
//   
public class SqlSessionFactoryProxy implements SqlSessionFactory{
 ...
 //  
  ThreadLocal<SqlSessionProxy> sqlSessionLocal = new ThreadLocal<>();
  
  public SqlSessionFactoryProxy(SqlSessionFactory original) {
    //Mybatis에서 제공하는 sqlsessionfactroy 구현체를 필드에 보관해 두었다가
    //  외부에서 요청이 들어오면 이객체를 사용하여 sqlsession객체를 만들어 준다.
    this.original = original;
  }
  
  // 트랜잭션 관리자가 트랜잭션을 시작시키면 
  // 기본적으로 auto commit이 false면 SqlSession 객체를 미리 만들어 스레드에 보관한다
  //    
  ...
  public void releaseForTransaction() {
    ...
    // SqlSession 객체를 닫는다.
      ...
      sqlSessionProxy.close(); //close() 할 때 SqlSession 객체를 닫을 것이다.
       ...
       // 사용한 자원을 해제시키다가 예외가 발생한 것을 무시한다.
    }
  }
  ...
  public SqlSession openSession() {
    SqlSession sqlSession = sqlSessionLocal.get();
      if(sqlSession != null)
        ...
        return original.openSession(true); //기본은 auto commit()이 true 이다.
  }
