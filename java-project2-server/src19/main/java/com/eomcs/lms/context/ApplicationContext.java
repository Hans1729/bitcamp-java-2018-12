...
public class ApplicationContext {
  ...
  private void prepareComponet() throws Exception {
    for (Class<?> clazz : classes) {
      ...
      if(obj != null) { // 제대로 생성 했으면 빈컨테이너에 저장한다.
        // 빈컨테이너에 Command 객체를 저장할 때 key 값은 Component 애노테이션의 value() 값으로 한다.
  ...
  private Object createInstance(Class<?> clazz) throws Exception { 
    // 파라미터로 주어진 클래스 정보를 가지고 인스턴스 정보를 생성한다.
    // => 기본 생성자를 알아 낸다.
    try {
    ...
    for(Constructor<?> c : constructors) {
      ...
      // => 생성자가 요구하는 타입의 파리미터 값이 저장소에 있는지 찾아 본다.
