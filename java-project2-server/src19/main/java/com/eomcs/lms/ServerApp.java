// 19단계: 애노테이션 적용
// => IoC 컨테이너가 객체를 만들 때 클래스의 애노테이션에서 객체 이름 추출하기 
// 
...
//       - 이런 메서드도(리스너)도 IoC의 한 예이다.
//       - 직접 호출 하는 것이 아니라 내부에 의해 호출되는 메서드라는 의미로 
...
//     => 기존의 name 필드로 객체 이름을 지정하는 대신에 애노테이션으로 객체 이름을 지정한다.
...
public class ServerApp {
  ...
  ApplicationContext beanCotainer;
  ...
  public void service() throws Exception {
    ...
    try (ServerSocket ss = new ServerSocket(8888)) {
      ...
      // ApplicationInitallizer가 준비한 ApplicationContext 를 꺼낸다
      
      beanCotainer = (ApplicationContext) context.get("applicationContext");
  ...
  class RequestHandlerThread extends Thread {
    ...
    public void run() {
      ...
      try (Socket socket = this.socket;
        ...
        // 클라이언트에게 응답하기
        // =>클라이언트 요청을 처리할 객치는 빈 컨테이너에서 꺼낸다
        Command commandHandler = (Command) beanCotainer.getBean(request);
