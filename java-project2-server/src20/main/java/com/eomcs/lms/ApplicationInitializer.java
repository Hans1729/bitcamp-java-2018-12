...
public class ApplicationInitializer implements ApplicationContextListener {
  ...
  public void contextInitialized(Map<String, Object> context) {
    try {
      ...
      BoardDao boardDao = 
          daoFactory.create(BoardDao.class);

      LessonDao lessonDao = 
          daoFactory.create(LessonDao.class);

      MemberDao memberDao = 
          daoFactory.create(MemberDao.class);
 
      PhotoBoardDao photoBoardDao = 
          daoFactory.create(PhotoBoardDao.class);

      PhotoFileDao photoFileDao = 
          daoFactory.create(PhotoFileDao.class);
      
      // ServerApp이 사용할 객체를 객체 보관소인 ApplicationContext 에 담아서
     //   =>Command 객체가 사용 할 DAO 구현체를 준비한다.
