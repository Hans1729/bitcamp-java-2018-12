...
// => 작업 
// 1) 클라이언트의 요청 작업을 처리하는 코드를 별로의 스레드 클래스로 분리한다.
...
public class ServerApp {
  ...
  static Set<String> seviceKeySet ;
  ...
  public static void main(String[] args) {
    ...
    seviceKeySet = serviceMap.keySet();

    try (ServerSocket serverSocket = new ServerSocket(8888)) {
      System.out.println("서버 시작!");

      while (true) {
        // 클라이언트 소캣을 꺼낸 후 스레드에게 전달한다.
        // 그리고 스레드를 실행시킨다.
