...
public class Controller02_1 {
  ...
  // 테스트:
  // http:...//app2/c02_1?name=kim&age=20
  @GetMapping
  @ResponseBody
  public String handler1(String name , int age) {
    ...
    // => Query String 의 값을 request handler 에서 받으려면
    //  아규먼트 를 선언하면 된다.
    // 아규먼트 앞에 @RequestParam을 붙여도되고
    
    return String.format("name=%s, age=%d", name , age);

  } 
  // 테스트:
  // http:...//app2/c02_1/kim/20
  @GetMapping("{name}/{age}")
  @ResponseBody
...
//테스트:
 ...
 public String handler3(@PathVariable("name") String name, @PathVariable("age") int age) {
   ...
   //  => URL에 포함된 값을 받으려면 request handler의 URL을 설정할 때
    //  다음의 문법으로 선언해야한다.
    // => .../{변수명}/{변수명}
    // 이렇게 선언된 값을 받으려면 다음과 같이 아규먼트를 선언해야 한다.
   // @PathVariable{변수명} String 아규먼트
    //
   return String.format("name=%s , age=%d", name , age);
 }
