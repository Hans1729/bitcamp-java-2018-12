...
public class Controller05_1 {
  ...
  // 2) Google Gson 라이브러리를 사용하여 JSON 형식의 콘텐트를 출력하기
  ...
  // 3) Google Gson 라이브러리를 사용하여 JSON 형식의 콘텐트를 출력하기
  //   =>   페이지 컨트롤러의 리턴 값이 String이 아니면
  //      프론트 컨트롤러는 
  //    Goole Gson 라이브러리나 JackSon 라이브러리를 사용하여
  //      자동으로 JSON 형식의 문자열로 만들어 클라이언트로 출력한다.
  //   => 단 Gson 또는 Jackson 라이브러리를 추가한다.
  //   => bulid.gradle 파일에서 gson 또는 jackson 라이브러리를 추가하는 부분의
  //        주석을 참고하라
  // 테스트:
  //   http://.../app2/c05_1/h3
  @GetMapping("h3")
