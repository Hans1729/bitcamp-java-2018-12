...
public class Test01 {
  ...
  public static void main(String[] args) {
   ...
   //  => 1) mavenrepository.com 에서 mariadb jdbc driver를 검색한다
    //   =>  2)  라이브러리 정보를 builkd.gradle 파일에 설정한다.
   //  =>  3) gradle을 이용하여 eclipse 설정 파일을 갱신한다.
    //         >gradle eclipse
    //            - 다운로드 받지 않은 라이브러리가 있다면 자동으로 서버에서 받을 것이다.
    //            - 라이브러리 정보가 변경되었다면해당 라이브러리를 서버에서 받을 것이다.
    //        4) 이클립스 프로젝트를 리프래시 한다.
    //        - 프로젝트에 mariad jdbc driver 라이브러리가 추가 되었는지 확인한다.
   // 
