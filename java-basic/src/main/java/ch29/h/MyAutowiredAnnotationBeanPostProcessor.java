...
// @Autowired 애노테이션을 처리해주는 역활을 한다.
//  스프링 IoC 컨테이너가 객체를 생성한 후 보고를 하면
//  이 클래스는 생성된 객체에서 @Autowired가 붙은 세터를 찾는다.
//  있다면 그 세터를 호출하여 의존 객체를 주입한다.
//  의존 객체가 없다면 그 의존 객체가 생성될 때까지
//  별도로 담아 둔다.
//  의존 객체 생성 되는 순간 즉시 별도로 담아 둔 그 객체에 대해 셋터를 호출할 것이다.
//
public class MyAutowiredAnnotationBeanPostProcessor  implements BeanPostProcessor{
  ...
  // 파라미터 값이 준비되지 않아서 호출이 연기된 @Autowired 메서드 기록한 맵
  ...
  private boolean isRegistered(Class<?> type)  /* BlackBox blackbox*/{
  ...
  private void callAutowiredMethod(Object paramsValue) {
    // 이 타입의 빈을 원하는 세터 목록을 꺼낸다.
  List<AutowiredMethod> setters = autowiredMethodMap.get(paramsValue.getClass());
  
  if(setters == null)// setter가 없다면 호출하지 않는다
    return;
  
        for(AutowiredMethod setter : setters) {
          try {
          setter.method.invoke(setter.object, paramsValue);
          }catch (Exception e) {
            e.printStackTrace();
          }
        }
